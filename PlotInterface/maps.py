...
class MapFigure(Figure, Basemap):
    ...
    def add(self, data, xgrid, ygrid, title, cbarlab, map_kwargs):
        self.subfigures.append((data, xgrid, ygrid, title, cbarlab, map_kwargs))
    ...
    def addGraticule(self, axes, mapobj, dl=2.):
    ...
    def addMapScale(self, mapobj):
        ...
        mapobj.drawmapscale(lonloc, latloc, midlon, midlat, scale_length, barstyle='fancy',
                            fontsize='x-small')
...
class MaskedContourMapFigure(FilledContourMapFigure):
    ...
    def subplot(self, axes, subfigure):
        ...
        subfigure = (masked_data, xgrid, ygrid, title, cbarlab, map_kwargs)
        super(MaskedContourMapFigure, self).subplot(axes, subfigure)

class ArrayMapFigure(MapFigure):
    """Array plot"""
    def add(self, data, xgrid, ygrid, title, cbarlab, map_kwargs):
        super(ArrayMapFigure, self).add(data, xgrid, ygrid, title,
                                        cbarlab, map_kwargs)
...
class MaskedArrayMapFigure(ArrayMapFigure):
    ...
    def subplot(self, axes, subfigure):
        from mpl_toolkits.basemap import maskoceans
        data, xgrid, ygrid, title, cbarlabel, map_kwargs = subfigure
...
class BarbMapFigure(MapFigure):
    ...
    def add(self, xdata, ydata, xgrid, ygrid, title, cbarlab, map_kwargs):
...
class HazardMap(MaskedContourMapFigure):
    ...
    def plot(self, data, xgrid, ygrid, title, cbarlab, map_kwargs):
        # Smooth the data to reduce 'lines-on-a-map' inferences:
        data = smooth(data, 40)
        self.add(data, xgrid, ygrid, title, cbarlab, map_kwargs)
...
def saveHazardMap(data, xgrid, ygrid, title, cbarlab, map_kwargs, filename):
    fig = HazardMap()
    
    fig.plot(data, xgrid, ygrid, title, cbarlab, map_kwargs)
    saveFigure(fig, filename)

def saveWindfieldMap(data, xgrid, ygrid, title, cbarlab, map_kwargs, filename):
    fig = WindfieldMap()
    fig.plot(data, xgrid, ygrid, title, cbarlab, map_kwargs)
    saveFigure(fig, filename)

def saveArrayMap(data, xgrid, ygrid, title, cbarlab, map_kwargs, filename):
    fig = ArrayMap()
    fig.plot(data, xgrid, ygrid, title, cbarlab, map_kwargs)
    saveFigure(fig, filename)
...
def main():
    ...
    saveHazardMap(vdata[4,:,:], xgrid, ygrid, title, cbarlab, map_kwargs, 'docs/hazard_map.png')
