...
class WriteFoliationCallback(object):
    ...
    ...     callback(datetime.datetime(2018,3,17), *[np.ones((len(lat),len(lon)))]*4, lon=lon, lat=lat)
    ...     callback(datetime.datetime(2018,3,18), *[np.full((3,3), fill_value=100)]*4, lon=lon[2:5], lat=lat[2:5])
    ...     callback(datetime.datetime(2018,3,20,12,0), *[np.ones((3,6))]*4, lon=lon[0:6], lat=lat[2:5])
    ...     callback.ds.close()
    ...
    [121.0, 900.0, 18.0]
    >>> (x.gust_speed.isel(time=2, lon=slice(0,6), lat=slice(2,5)).values == 1).all()
    True
    """
    ...
    def __init__(self, filename, gridLimit, resolution, margin=0, maxchunk=256, wraps=None):
        ...
        self.affine = ~(affine.Affine.translation(xoff=gridLimit['xMin']-margin,
        ...
        # declare shapes
        root.createDimension('time', size=None) # growable
    ...
    def __call__(self, time, gust, Ux, Uy, P, lon, lat):
        ...
        if not t: # then initialise
            self.time.units = "days since " + time.date().isoformat()
